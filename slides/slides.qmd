---
title: "R package development with GitHub Pages<br> and `pkgdown`"
subtitle: "R-Ladies Ottawa, 2025"
author: "Taught by *[Melissa Van Bussel](https://www.melissavanbussel.com)*"
execute:
  echo: true
  eval: false
  message: false
...
format:
  revealjs:
    ...
    menu: false
    lang: en
    footer: "[https://melissavanbussel.github.io/rlo2025](https://melissavanbussel.github.io/rlo2025/index.html)"
    slide-number: c/t
...
# {background-image="thank_you_slide.jpg"}
...
* The link in the footer contains the `pkgdown` site we will be working towards building together
* You can access the slides at [https://melissavanbussel.github.io/rlo2025/slides.html](https://melissavanbussel.github.io/rlo2025/slides.html), which are also embedded on the `pkgdown` site
* You can access the GitHub repo by clicking the GitHub icon in the navbar, or by going to [github.com/melissavanbussel/rlo2025](https://www.github.com/melissavanbussel/rlo2025)

:::

## How to get the most out of today's session

* **Please participate!** This is a 2-hour workshop, which is a long time to stay focused. The more you participate, the more fun it will be for everyone. ☺️
...
* 🎨 An understanding of how to customize some components of `pkgdown` websites

## The final product we will create

<iframe src="https://melissavanbussel.github.io/rlo2025/index.html" width="1000" height="450"></iframe>

## Background knowledge

* Participants should have a GitHub account, but don’t need prior experience with `pkgdown` or GitHub Pages.
...
[If not using Posit Cloud] Please ensure you have all of the following software installed (and updated to the most recent versions):

* [R v4.5.1](https://cran.rstudio.com/)
* [Rtools v4.5](https://cran.r-project.org/bin/windows/Rtools/)
* [RStudio v2025.09.1+401](https://posit.co/download/rstudio-desktop/)
* [Git v2.51.1](https://git-scm.com/downloads)

## Posit Cloud

* Alternatively, you can use the Posit Cloud link that was shared by email with the registration list
* After creating an account and joining the workspace, click on `Content > New project from template`
* Select `rlo_blank_project` if you want to **follow along live** and type everything yourself, or select `rlo2025` if you want to **just see the finished project**

## Installing packages

* If you used one of the project templates from Posit Cloud, the packages you need should already be installed
* If you're not using Posit Cloud, you'll want to run the code below to install the necessary packages: 

```{r}
#| eval: false
install.packages(c("devtools", "reticulate", "jsonlite", "testthat", "covr", "quarto"))
```
...
library(devtools)
library(pkgdown)
create_package("/cloud/project")
```

* Say **yes** to overwriting the existing `.Rproj` file
* Then click on **"Don't save"** when asked if you would like to save the workspace image
* You may need to **re-load the two packages above** when RStudio restarts
...
```{r}
#' Print hello R-Ladies
#'
#' This function just prints hello R-Ladies.
#'
#' @param x This argument does nothing.
#'  If it did do something, it might require more words to explain.
#'
#' @return Prints `"Hello, R-Ladies"` to the console
#'
#' @examples
#' print_hello_rladies(x = 1)
#'
#' @export
print_hello_rladies <- function(x) {
  print("Hello, R-Ladies!")
}
```
...
## Repeat for the `print_hello_rladies` function
...
We should see no errors, warnings, or notes when we run `check()`!

```{r}
check()
```

* [**Note #1:** If running on Posit Cloud, you will always see a NOTE which can be ignored, that says `checking for future file timestamps ... NOTE unable to verify current time`]
* [**Note #2:** If you're running the finished version of the project on Posit Cloud, you will need to click `Build > Install` first before you run `check()`.]
...
Let's **commit** all the changed files and **push** the changes to GitHub.
...
```{r}
# If using RStudio on your laptop:
build_site()

# If using RStudio on Posit Cloud:
...
* [**Note for Posit Cloud Users:** Use the upload button, then run `use_logo`, and then you can delete the image you uploaded.]
...
url: https://melissavanbussel.github.io/rlo2025/
template:
  bootstrap: 5
  bslib:
    bootswatch: zephyr
...
## We can do this for any CSS class!

Let's change the navbar colour for light and dark mode: 
...
footer:
  ...
  components:
    developed_by: "Developed by [Melissa Van Bussel](https://melissavanbussel.com/) :computer: for the [R-Ladies Ottawa 2025 `pkgdown` workshop](https://www.meetup.com/rladies-ottawa/events/311256371/)."
...
# Part 3: BONUS topics {background-image="part3.png"}
...
```{r}
use_vignette(name = "my_vignette.qmd", title = "Tutorial Title")
```

* `Build > Install > Clean and install`
* Ensure Quarto is installed (including the `quarto` package in R)
* Contents can be pasted from [here](https://github.com/melissavanbussel/rlo2025/blob/main/vignettes/my_vignette.qmd)
  ...
  components:
    ...
    rd_files:
      ...
      menu:
        ...
        - text: "    Print Hello R-Ladies"
          href: reference/print_hello_rladies.html
```
...
## Example: Quarto slides inside of a package
...
```yaml
        text: |
          ![](https://img.shields.io/github/r-package/v/melissavanbussel/rlo2025?label=rlo2025)<br>
          ![](https://img.shields.io/github/actions/workflow/status/melissavanbussel/rlo2025/pkgdown.yaml)<br>
```
...
Let's add a test coverage badge: ![](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fmelissavanbussel.github.io%2Frlo2025%2Fcoverage.json&query=%24.coverage&label=test%20coverage)
    ...
    components:
      badges:
        ...
        text: |
          ![](https://img.shields.io/github/r-package/v/melissavanbussel/rlo2025?label=rlo2025)<br>
          ![](https://img.shields.io/github/actions/workflow/status/melissavanbussel/rlo2025/pkgdown.yaml)<br>
          ![](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fmelissavanbussel.github.io%2Frlo2025%2Fcoverage.json&query=%24.coverage&label=test%20coverage)<br>
```
...
Please use [this form](https://rladies-ottawa.github.io/feedback.html) to provide anonymous feedback about today's workshop. 

## Photos from today's event

Please upload your photos on the `#photos` channel on Slack, which you can find by using the QR code that was printed out!

## Upcoming events

Stay tuned on Meetup for our event in November!
